...
class PDFGenerator:
    ...
    def format_schedule_data(self, db_schedules):
        """
        Format database schedule data for PDF generation
        
        Args:
            db_schedules: List of tuples from database (staff_name, day, schedule_date, is_working, start_time, end_time)
        
        Returns:
            List of tuples formatted for PDF generation and week_dates dictionary
        """
        try:
            print(f"DEBUG: format_schedule_data - input: {db_schedules}")
            ...
            for record in db_schedules:
                ...
                # Store week dates
                if schedule_date and day and day not in week_dates:
                    week_dates[day] = schedule_date
                    print(f"DEBUG: Added week date for {day}: {schedule_date}")
                
                # Add to formatted data
                formatted_data.append((staff_name, day, schedule_date, is_working, start_time or "", end_time or ""))
            
            print(f"DEBUG: format_schedule_data - formatted_data: {formatted_data}")
            print(f"DEBUG: format_schedule_data - week_dates: {week_dates}")
            
            return formatted_data, week_dates
            
        except Exception as e:
            print(f"DEBUG: format_schedule_data Error: {e}")
            import traceback
            traceback.print_exc()
            raise e 